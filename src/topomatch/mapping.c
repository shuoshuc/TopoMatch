...
void dump_solution(const char *filename, int *arr, int size)
{
  ...
  {
    fprintf(fp, "%d", arr[i]);
    // Only print a comma if it's NOT the last element
    if (i < size - 1)
    {
      fprintf(fp, ", ");
    }
                 ...
                 char *sol_filename)
{
  ...
  dump_solution(sol_filename, sol->sigma, aff_mat->order);
...
int main(int argc, char **argv)
{
  ...
  /*single character <=> flag ;
    character + ':' <=> option*/
  while ((c = getopt(argc, argv, "hsnkedt:x:c:b:v:m:o:p:f:l:g:a:w:")) != -1)
  {
    switch (c)
    {
    case 'a':
  ...
  return test_mapping(arch_filename, arch_file_type, com_filename, bind_flag, bind_filename,
                      opt_topo_flag, metric, oversub_fact, display_solution_flag, sol_filename);
}
