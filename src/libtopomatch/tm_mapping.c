...
tm_affinity_mat_t * new_affinity_mat(double **mat, double *sum_row, int order, long int nnz, double max_val);
...
long int init_mat_long(char *filename,int N, double **mat, double *sum_row, double *max_val);
long int init_mat_mmap(char *filename,int N, double **mat, double *sum_row, double *max_val);
char *parse_line(int i, double **mat, double *sum_row, int N, char *data, char *filename, long int *nnz, double *max_val);
...
long int init_mat_mmap(char *filename, int N, double **mat, double *sum_row, double *max_val){
  ...
  i = 0;
  while(i<N){
    data = parse_line(i, mat, sum_row, N, data, filename, &nnz, max_val);
    i++;
  }
...
tm_affinity_mat_t * tm_build_affinity_mat(double **mat, int order){
  ...
  return new_affinity_mat(mat, sum_row, order, nnz, max_val);
}
...
int nb_lines(char *filename){
  ...
  if((long unsigned int) filesize < (long unsigned int) nb_threads){
    return nb_lines_pos(filename, 0, filesize); 
  }

  tm_verbose_fprintf(DEBUG, verbose_stdout,"filesize= %lu, nb_thread=%d, ratio= %lu\n", (long unsigned int) filesize, nb_threads, (long unsigned int)(filesize/nb_threads)); 
  ...
  for(id=0 ; id < nb_threads ; id++){
    ...
    tm_verbose_fprintf(DEBUG2, verbose_stdout,"%d/%d (%ld->%ld)\n", id, nb_threads, inf[id], sup[id]); 
    args[0]=(void*)(inf+id);
    args[1]=(void*)(sup+id);
    ...
    tm_verbose_fprintf(DEBUG2,verbose_stdout,"Executing %p\n",(void *)works[id]);
...
long int init_mat_long(char *filename,int N, double **mat, double *sum_row, double *max_val){
  ...
  while(i<N){
    ...
    parse_line(i, mat, sum_row, N, line, filename, &nnz, max_val);
    i++;
  }
...
tm_affinity_mat_t *tm_load_aff_mat(char *filename)
{
  ...
  if (get_filesize(filename) > 1024*1024*1014) {
    nnz = init_mat_long(filename,order, mat, sum_row, &max_val);
    tm_verbose_fprintf(DEBUG,verbose_stdout,"New parser\n");


  }else{
    nnz = init_mat_mmap(filename,order, mat, sum_row, &max_val);
  ...
  duration = TOC;
  tm_verbose_fprintf(TIMING, verbose_stdout, "Reading %s in %.2fms\n", filename, duration*1000);
