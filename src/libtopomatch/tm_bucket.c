...
{
  tm_verbose_fprintf(INFO, verbose_stdout, "\tb.bucket=%p\n",(void *)b->bucket);
  tm_verbose_fprintf(INFO, verbose_stdout, "\tb.bucket_len=%d\n",(int)b->bucket_len);
  tm_verbose_fprintf(INFO, verbose_stdout, "\tb.nb_elem=%d\n",(int)b->nb_elem);
}
...
void check_bucket(bucket_t *b,double **tab,double inf, double sup)
{
  ...
  for( k = 0 ; k < b->nb_elem ; k++ ){
    ...
    if((tab[i][j] < inf) || (tab[i][j] > sup)){
      tm_verbose_fprintf(CRITICAL,verbose_stderr,"[%ld] (%d,%d):%f not in [%f,%f]\n",(unsigned long)k,i,j,tab[i][j],inf,sup);
      exit(-1);
    }
  }
}
...
void display_pivots(bucket_list_t bucket_list)
{
  ...
  if(verbose_level>DEBUG){
    for( i = 0 ; i < bucket_list->nb_buckets-1 ; i++)
      tm_verbose_fprintf(DEBUG, verbose_stdout, "pivot[%d]=%f\n",i,bucket_list->pivot[i]);
  }
}
...
void display_bucket_list(bucket_list_t bucket_list)
{
  ...
  for(i = 0 ; i < bucket_list->nb_buckets ; i++){
    ...
    if(verbose_level >= INFO){
      tm_verbose_fprintf(INFO,verbose_stdout,"Bucket %d:\n",i);
...
void add_to_bucket(int id,int i,int j,bucket_list_t bucket_list)
{
  ...
  if( bucket->bucket_len == bucket->nb_elem ){
    ...
    size = N*(N/n);
    /* display_bucket(bucket);*/
    tm_verbose_fprintf(DEBUG,verbose_stdout,"Extending bucket %d (%p) from size %llu to size %llu!\n",id,(void*)bucket->bucket, (unsigned long long) bucket->nb_elem, (unsigned long long)bucket->nb_elem+size);
  ...
  if(bucket->bucket_len <= bucket->nb_elem ){
    tm_verbose_fprintf(CRITICAL, verbose_stderr,  "Error bucket->bucket_len=%llu < bucket->nb_elem=%llu\n",bucket->bucket_len, bucket->nb_elem);
    exit(-1);
  }
...
void  built_pivot_tree(bucket_list_t bucket_list)
{
  ...
  if(verbose_level >= DEBUG){
    ...
    for(i=0;i<2*n;i++)
      fprintf(*verbose_stdout, "%d:%f\t",i,pivot_tree[i]);
...
void partial_sort(bucket_list_t *bl,double **tab,int N)
{
  ...
  if( N <= 0){
    tm_verbose_fprintf(ERROR,verbose_stderr,"Error: trying to group a matrix of size %d<=0!\n",N);
    return;
  }
  ...
  if(!is_power_of_2(nb_buckets)){
    tm_verbose_fprintf(ERROR,verbose_stderr,"Error! Paramater nb_buckets is: %d and should be a power of 2\n",nb_buckets);
    exit(-1);
  }
  ...
  n = pow(2,nb_buckets);
  tm_verbose_fprintf(INFO,verbose_stdout,"N=%d, n=%ld\n",N,n);
  ...
  for( k =  0 ; k < n ; k++ ){
    ...
    tm_verbose_fprintf(DEBUG2,verbose_stdout,"i=%d, j=%d\n",i,j);
    assert( i != j );
    assert( i < j );
    assert( i < N );
    assert( j < N );
  ...
  if(verbose_level >= DEBUG2)
    for(k=0;k<n;k++){
      i=sample[2*k];
      j=sample[2*k+1];
      tm_verbose_fprintf(DEBUG2, verbose_stdout, "%f\n",tab[i][j]);
    }
...
void next_bucket_elem(bucket_list_t bucket_list,int *i,int *j)
{
  ...
  */
  while( bucket->nb_elem <= bucket_list->bucket_indice ){
    ...
    //printf("nb_elem: %d, indice: %d, bucket_id: %d\n",(int)bucket->nb_elem,bucket_list->bucket_indice,bucket_list->cur_bucket);
    tm_verbose_fprintf(DEBUG,verbose_stdout,"### From bucket %d to bucket %d\nnb_elem: %d, indice: %d, bucket_id: %d\n",bucket_list->cur_bucket-1,bucket_list->cur_bucket, (int)bucket->nb_elem,bucket_list->bucket_indice,bucket_list->cur_bucket);
  }
  ...
  if(!bucket->sorted){
    ...
    double duration = TOC;
    tm_verbose_fprintf(INFO,verbose_stdout,"Sorting %llu elem in %fs.\n",bucket->nb_elem,duration);
...
int try_add_edge(tm_tree_t *tab_node, tm_tree_t *parent,int arity,int i,int j,int *nb_groups)
{
  ...
  default:
    ...
    tm_verbose_fprintf(ERROR,verbose_stderr,"Cannot handle arity %d\n",parent->arity);
    exit(-1);
  }
}
		       ...
		       int arity,int M)
{
  ...
  tm_verbose_fprintf(INFO,verbose_stdout,"starting sort of N=%d elements\n",N);

  TIC;
  partial_sort(&bucket_list,mat,N);
  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"Partial sorting=%fs\n",duration);
  if(verbose_level >= DEBUG)
    display_pivots(bucket_list);
  ...
  if(verbose_level >= INFO){
    ...
    while( l < M ){
      ...
      if(verbose_level >= DEBUG)
	fprintf(*verbose_stdout,"elem[%d][%d]=%f ",i,j,mat[i][j]);
      gr1_1 += TOC;
      TIC;
  ...
  gr1=TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"Grouping phase 1=%fs (%fs+%fs) \n",gr1, gr1_1, gr1_2);

  tm_verbose_fprintf(DEBUG,verbose_stdout,"l=%d,nb_groups=%d\n",l,nb_groups);
  ...
  tm_verbose_fprintf(INFO,verbose_stdout,"Grouping phase 2=%fs\n",gr2);
  ...
  gr3=TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"Grouping phase 3=%fs\n",gr3);
  /* printf("val=%f\n",val);exit(-1); */

  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"Grouping =%fs\n",duration);

  tm_verbose_fprintf(DEBUG,verbose_stdout,"Bucket: %d, indice:%d, val=%f\n",bucket_list->cur_bucket,bucket_list->bucket_indice,val);
