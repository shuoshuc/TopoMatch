...
double display_sol_sum_com_scotch(tm_topology_scotch_t *topology, tm_affinity_mat_t *aff_mat, int *sigma)
{
  ...
  int N = aff_mat->order;
  int dis;
  /* double threshold = aff_mat->max_val * tm_get_sparse_factor(); /\*select the value of the communication matrix the above which they will be in the result (Scotch graph) *\/ */
...
double tm_display_solution(tm_topology_t *topology, tm_affinity_mat_t *aff_mat, tm_solution_t *sol,
  ...
  if(verbose_level >= DEBUG){
    ...
    for( i = 0 ; i < sol->k_length ; i++ ){
   
      if(k[i][0] != -1){
	tm_verbose_fprintf(DEBUG, verbose_stdout, "\tProcessing unit %lu: ",i);
...
void tm_display_other_heuristics(tm_topology_t *topology, tm_affinity_mat_t *aff_mat, tm_metric_t metric)
{
  ...
  if (topology->topology_type == TM_TOPOLOGY_TREE){
    ...
    printf("RR: ");
    display_sol(topology, aff_mat, topology->topology.scotch->node_id, metric);
  }
  /* double duration; */
  /* CLOCK_T time1,time0; */
  /* CLOCK(time0); */
