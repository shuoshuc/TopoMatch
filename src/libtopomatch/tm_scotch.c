...
double sparse_factor = 0.5;

void tm_set_sparse_factor(float val){
  sparse_factor = val;
}

float tm_get_sparse_factor(void){
...
void symmetrize_com_mat(com_mat_t *com_mat){
  int i,j;
  int n = com_mat->n;
  ...
  if(j == n){ /*if we have not break then first line is first column*/
    /* printf("Symmetric\n"); */
    return;
  }
  ...
  com_mat->max_val = max_val;
}

SCOTCH_Graph* com_mat_to_scotch_graph(com_mat_t *com_mat, int n){
  double **mat = com_mat->comm;
  SCOTCH_Num vertnbr = com_mat->n;// number of vertices
  ...
  double threshold = com_mat->max_val * tm_get_sparse_factor(); /*select the value of the communication matrix the above which they will be in the result (Scotch graph) */
...
int find_oversub(int *sigma, int size , int *nb_proc){
  ...
  for(i=0 ; i<size ; i++)
    if(sigma[i] + 1 > *nb_proc)
      *nb_proc = sigma[i] + 1; /*The k array needs one extra element because we access k[sigma[i]], so its size must be sigma[i] + 1.*/
...
SCOTCH_Num *scotch_partitionning(SCOTCH_Arch archdat, tm_affinity_mat_t *aff_mat, SCOTCH_Num *parttab_len, int oversub_fact){
  ...
  com_mat->max_val = aff_mat->max_val;

  graphptr = com_mat_to_scotch_graph(com_mat, arch_size * oversub_fact);

  SCOTCH_graphSize (graphptr, &vertnbr, NULL);
  tm_verbose_fprintf(INFO, verbose_stdout, "Comm mat converted to Scotch graph (vertnb=%lld)\n",(long long)vertnbr);
...
int *extract_id_from_sigma_sorted(int *sigma, int n){
  int *res = (int *)MALLOC(n * sizeof(int));
  memcpy(res, sigma, n*sizeof(int));
  qsort(res, n, sizeof(int), int_cmp);
  return res;
}
...
tm_solution_t *build_mapping_with_scotch(tm_topology_scotch_t *topology, tm_affinity_mat_t *aff_mat) {
  ...
  if(nb_constraints){
    ...
    parttab = scotch_partitionning(topology->arch, aff_mat, &parttab_len,  oversub_max);
  }
  ...
  oversub_real  = find_oversub(sigma, nb_processes, (int*)&npu); // Here we can chnage the number of provessing unit as beeing the "maximum value of sigma"+1 (because sigma cannot always be numbered from 0 to n due to sub arch features)
  k             = (int**) MALLOC(sizeof(int*) * npu);

  if(oversub_real > oversub_max){
    tm_verbose_fprintf(ERROR, verbose_stderr, "Scotch oversubsribed the architecture by a factor of %d while the maximum factor is set to %d.\nYou can fix it by increasing the oversubscribing factor to %d (e.g. through -o option, or the tm_enable_oversubscribing function of the library)\nExiting.\n",oversub_real, oversub_max, oversub_real);
    exit(-1);
  }
  
  topology->node_id = extract_id_from_sigma_sorted(sigma, nb_processes);
