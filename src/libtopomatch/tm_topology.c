...
void  tm_set_mapping_strat(tm_mapping_strat_t strat){
  if(strat > TM_STRAT_SCOTCH){
    ...
    tm_verbose_fprintf(ERROR,verbose_stderr,"Falling back to TopoMatch strategy!\n");
...
/* Not Called */
tm_numbering_t  tm_get_numbering(void){
  return numbering;
}
...
void tm_free_topology(tm_topology_t *topology)
{
  ...
  if (topology->topology_type == TM_TOPOLOGY_TREE){
    ...
    FREE(topology->topology.scotch->node_id);
...
tm_topology_t *tm_load_topology(char *arch_filename, tm_file_type_t arch_file_type){
  ...
  case   TM_FILE_TYPE_TGT:
    ...
    if (tm_get_strat()==TM_STRAT_SCOTCH ||!is_tleaf(arch_filename)){
      ...
      tm_verbose_fprintf(DEBUG,verbose_stdout,"Storing topology in TopoMatch format\n");
...
#if HAVE_LIBSCOTCH
    int size = SCOTCH_archSize(&topology->topology.scotch->arch);
