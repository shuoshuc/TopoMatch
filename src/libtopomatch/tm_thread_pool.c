...
int bind_myself_to_core(hwloc_topology_t topology, int id){
  ...
  case COMPACT:
    ...
    break;
  default:
    tm_verbose_fprintf(WARNING,verbose_stdout,"Wrong scheduling policy. Using COMPACT\n");
  }
  tm_verbose_fprintf(INFO,verbose_stdout,"Mapping thread %d on core %d\n",id,my_core);
  ...
  if (obj) {
    ...
    */
    ...
    if (binding_res == -1){
      ...
      tm_verbose_fprintf(WARNING,verbose_stdout,"Thread %d couldn't bind to cpuset %s: %s (This thread is not bind to any core...)\n", my_core, str, strerror(error));
    ...
    tm_verbose_fprintf(WARNING,verbose_stdout,"No valid object for core id %d!\n",my_core);
    return 0;
  }
}
...
thread_pool_t *create_threads(void){
  ...
  if (depth == -1 ) {
    ...
    tm_verbose_fprintf(CRITICAL,verbose_stderr,"Error: HWLOC unable to find the depth of the topology of this node!\n");
    exit(-1);
  }
  ...
  tm_verbose_fprintf(INFO,verbose_stdout,"nb_threads = %d\n",nb_threads);
  ...
  for (i=0;i<nb_threads;i++){
    ...
    if (pthread_create (pool->thread_list+i, NULL, thread_loop, local+i) < 0) {
      tm_verbose_fprintf(CRITICAL,verbose_stderr,"pthread_create error for exec thread %d\n",i);
      return NULL;
    }
  }
  return pool;
...
thread_pool_t *get_thread_pool(void){;
  if (pool == NULL)
...
void terminate_thread_pool(void){
  ...
  if(pool){
	...
	tm_verbose_fprintf(WARNING,verbose_stdout,"Working list of thread %d not empty!\n",id);
    }
...
work_t *create_work(int nb_args, void **args, void (*task) (int, void **, int)){
  ...
  tm_verbose_fprintf(DEBUG,verbose_stdout,"work %p created\n",(void *)work);
  return work;
}
