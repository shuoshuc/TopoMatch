#ifndef __TOPOMATCH_H__
#define __TOPOMATCH_H__
...
extern FILE **verbose_stderr;
extern FILE **verbose_stdout;
...
/* Mapping type to prefer Scotch or TopoMatch */
typedef enum{
  TM_STRAT_TM     = 0,
  TM_STRAT_SCOTCH = 1
} tm_mapping_strat_t;
...
typedef struct {
  ...
  SCOTCH_Arch arch;
  #endif
  int *node_id;           /* ID of the nodes of the topology to be built once sigma is computed*/
  tm_common_topology_t *common;
}tm_topology_scotch_t;


typedef struct{
  tm_topology_type_t topology_type;
  union{
    tm_topology_tree_t *tree;
    tm_topology_scotch_t *scotch;
  }topology;
}tm_topology_t;
...
/* compute the mapping according to the topology and the affinity*/
...
/* Specify preffered mapping strategy fot tree topologies, Scotch (TM_STRAT_SCOTCH) or TopoMatch (TM_STRAT_TM)*/
void  tm_set_mapping_strat(tm_mapping_strat_t new_val);
tm_mapping_strat_t  tm_get_strat(void);
...
/* Specify the sparse factor to trim small values (i.e. values <= max_val*sparse_factor are consiered 0 for Scotch)*/
