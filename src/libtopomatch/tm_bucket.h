...
typedef struct{
  ...
  unsigned long long bucket_len; /* allocated size in the heap . ull : required for very large files */
  unsigned long long nb_elem;    /* number of usefull elements (nb_elem should be lower than bucket_len) */
  int sorted;
}bucket_t;
