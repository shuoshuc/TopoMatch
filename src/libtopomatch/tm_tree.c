...
int  exhaustive_search(group_list_t **tab_group, int n, int arity, int solution_size, double *best_val, group_list_t **best_selection);
...
int thread_derecurs_exhaustive_search(group_list_t **tab_group, int i, int nb_groups, int arity, int depth, int solution_size,double val, double *best_val, group_list_t **selection, group_list_t **best_selection, int8_t **indep_mat, pthread_mutex_t *lock, int thread_id, int *tab_i, int start_depth);
//
...
void display_selection (group_list_t**, int, int, double, int level);
void display_grouping (tm_tree_t *, int, int, double, int level);
...
double super_fast_grouping(tm_affinity_mat_t *, tm_tree_t *, tm_tree_t *, int, int);
...
void create_dumb_tree(tm_tree_t *, int, tm_topology_tree_t *);
void complete_tab_node(tm_tree_t **, int, int, int, tm_topology_tree_t *);
void set_deb_tab_child(tm_tree_t *, tm_tree_t *, int);
tm_tree_t *build_level_topology(tm_tree_t *, tm_affinity_mat_t *, int, int, tm_topology_tree_t *, double *, double *);
int check_constraints(tm_topology_tree_t  *, int **);
tm_tree_t *bottom_up_build_tree_from_topology(tm_topology_tree_t *, tm_affinity_mat_t *, double *, double *);
...
static long bucket_grouping_threshold = 30;

void tm_set_bucket_grouping_threshold(long val){
  bucket_grouping_threshold = val;
}

long tm_get_bucket_grouping_threshold(void){
  return bucket_grouping_threshold;
}
...
void partial_aggregate_aff_mat (int nb_args, void **args, int thread_id){
  ...
  if(nb_args != 8){
    tm_verbose_fprintf(ERROR,verbose_stderr,"Thread %d: Wrong number of args in %s: %d\n", thread_id, __func__ , nb_args);
    exit(-1);
  }

  tm_verbose_fprintf(INFO,verbose_stdout,"Aggregate in parallel (%d-%d)\n", inf, sup-1);
...
tm_affinity_mat_t *aggregate_aff_mat(tm_tree_t *tab_node, tm_affinity_mat_t *aff_mat, int M)
{
  ...
  return new_affinity_mat(new_mat, sum_row, M, nnz, 0);
}
...
void display_tab(double **tab, int mat_order)
{
  ...
  for( i = 0 ; i < mat_order ; i++ ){
	...
	fprintf(*verbose_stdout, "%g ", tab[i][j]);
      else
	fprintf(*verbose_stderr, "%g ", tab[i][j]);
      line += tab[i][j];
    }
    total += line;
    /* printf(": %g", line);*/
    if(verbose_level >= WARNING)
      ...
      fprintf(*verbose_stderr, "\n");
  }
  if(verbose_level >= WARNING)
    fprintf(*verbose_stdout,"Total: %.2f\n", total);
  else
    fprintf(*verbose_stderr,"Total: %.2f\n", total);
...
void add_to_list(group_list_t *list, tm_tree_t **cur_group, int arity, double val)
{
  ...
  if(verbose_level > DEBUG2){
    tm_verbose_fprintf(DEBUG2, verbose_stdout,"");
    for( i = 0 ; i < arity ; i++ ){
      fprintf(*verbose_stdout,"cur_group[%d]=%d ", i, cur_group[i]->id);
    }
    fprintf(*verbose_stdout,": %f\n", val);
  }
  /*printf("\n");*/
...
/* Not Called */
void display_selection (group_list_t** selection, int M, int arity, double val, int level)
{
  int i, j;
  double local_val = 0;
  ...
  for( i = 0 ; i < M ; i++ ) {
    for( j = 0 ; j < arity ; j++ )
      fprintf(*verbose_stdout, "%d ", selection[i]->tab[j]->id);
    fprintf(*verbose_stdout, "(%d)-- ", selection[i]->id);
    local_val+=selection[i]->val;
  }
  fprintf(*verbose_stdout, ":%f -- %f\n", val, local_val);
}

/* Not Called */
void display_grouping (tm_tree_t *father, int M, int arity, double val, int level)
{
  int i, j;
  if(verbose_level < level)
    return;

  tm_verbose_fprintf(level, verbose_stdout, "Grouping :");

  for( i = 0  ; i < M ; i++ ){
    for( j = 0 ; j < arity ; j++ )
      fprintf(*verbose_stdout, "%d ", father[i].child[j]->id);
    fprintf(*verbose_stdout, "-- ");
  }
  fprintf(*verbose_stdout, ":%f\n", val);
}
...
int recurs_select_independent_groups(group_list_t **tab, int i, int n, int arity, int d, int M, double val, double *best_val, group_list_t **selection, group_list_t **best_selection)
{
  ...
  */
  ...
  if( d == M ){
    display_selection(selection, M, arity, val, DEBUG);
    if( val < *best_val ){
      *best_val = val;
      for( i = 0 ; i < M ; i++ )
			       ...
			       group_list_t **best_selection, int bound, double max_duration)
{
  ...
  if(verbose_level>=DEBUG2){
    for(i=0;i<n;i++){
      tm_verbose_fprintf(DEBUG2, verbose_stdout,"");
      for(j=0;j<arity;j++){
	fprintf(*verbose_stdout, "%d ", tab_group[i]->tab[j]->id);
      }
      fprintf(*verbose_stdout, " : %f\n", tab_group[i]->val);
    }
  }
  ...
  display_selection(best_selection, M, arity, *best_val, INFO);
  return 0;
}
...
int thread_derecurs_exhaustive_search(group_list_t **tab_group, int i, int nb_groups, int arity, int depth, int solution_size,
 ...
 stack:
  ...
  if( depth == solution_size ){
    display_selection(selection, solution_size, arity, val, DEBUG);
    if( val < *best_val ){
...
void  partial_exhaustive_search(int nb_args, void **args, int thread_id){
  ...
  double duration = TOC;
  pthread_mutex_unlock(lock);
  tm_verbose_fprintf(INFO,verbose_stdout,"Thread %d done in %.3f!\n" , thread_id, duration);
}
...
double *build_bound_array(double *tab, int n){
  ...
  if(verbose_level>=DEBUG){
    ...
    fprintf(*verbose_stdout, "T(%d): ",n);
    for(i = 0; i<n ; i++)
      fprintf(*verbose_stdout,"%.0f ",tab[i]);
    fprintf(*verbose_stdout,"\n");
  }
  bound[0] = 0;
  bound[1] = tab[0];
...
int thread_exhaustive_search(group_list_t **tab_group, int nb_groups, int arity, int solution_size, double *best_val,
  ...
  for( i=nb_groups-1 ; i>=0 ; i--){
    ...
    if(verbose_level>=DEBUG){
    tm_verbose_fprintf(DEBUG, verbose_stdout, "-->(%d--%d) %.0f: ", i, nb_groups-i-1, tab_group[i]->val);
      for(j=1 ; j<nb_groups-i;j++){
	fprintf(*verbose_stdout, "%.0f - ",tab_group[i]->bound[j]);
      }
      fprintf(*verbose_stdout, "\n");
    }
  }
  ...
  display_selection(best_selection, solution_size, arity, *best_val, INFO);
		       ...
		       group_list_t **best_selection)
{
  ...
  selection = (group_list_t **)MALLOC(sizeof(group_list_t*)*solution_size);
  for( i = 0 ; i < n ; i++ ){
    tm_verbose_fprintf(INFO, verbose_stdout, "\r%.2f%% of search space explored...", (100.0*i)/n);
    selection[0] = tab_group[i];
    val = tab_group[i]->val;
  ...
  display_selection(best_selection, solution_size, arity, *best_val, INFO);
  duration = TOC;
  tm_verbose_fprintf(INFO, verbose_stdout, "Seq exhaustive search = %g\n",duration);
...
void list_to_tab(group_list_t *list, group_list_t **tab, int n)
{
  ...
  for( i = 0 ; i < n ; i++ ){
    if(!list){
      tm_verbose_fprintf(CRITICAL,verbose_stderr,"Error not enough elements. Only %d on %d\n", i, n);
      exit(-1);
    }
    tab[n-i-1] = list;
    tab[n-i-1]->id = n-i-1;
  ...
  if(list){
    tm_verbose_fprintf(CRITICAL,verbose_stderr,"Error too many elements\n");
    exit(-1);
  }
}
...
void display_tab_group(group_list_t **tab, int n, int arity)
{
  ...
  for( i = 0 ; i < n ; i++ ){
    for( j = 0 ; j < arity ; j++ )
      fprintf(*verbose_stdout, "%d ", tab[i]->tab[j]->id);
    fprintf(*verbose_stdout, ": %.2f %.2f\n", tab[i]->val, tab[i]->wg);
  }
}
...
void compute_weighted_degree(group_list_t **tab, int n, int arity)
{
  int i, j;
  for( i = 0 ; i < n ; i++){
    tab[i]->sum_neighbour = 0.0;
    tab[i]->nb_neighbour = 0;
  }
  for( i = 0 ; i < n ; i++ ){
    /* printf("%d/%d=%f%%\n", i, n, (100.0*i)/n); */
    for( j = i+1 ; j < n ; j++ ){
  ...
  /* double sn = tab[0]->sum_neighbour; */
  /* printf("nbg= %d (%d)\n",n, independent_tab(tab[4]->tab, tab[6]->tab, arity));  */
  for( i = 0 ; i < n ; i++){
    /* for(j=0;j<arity;j++){ */
    /*   printf("%d ",tab[i]->tab[j]->id); */
    /* } */
    /* if(sn!=tab[i]->sum_neighbour) */
    /*   printf("sn[0]= %f, sn[%d]= %f (%f) ",sn, i, tab[i]->sum_neighbour,tab[i]->val);   */
    if(tab[i]->nb_neighbour == 0)
      tab[i]->wg = FLT_MAX;
    else
      tab[i]->wg = tab[i]->sum_neighbour/tab[i]->nb_neighbour;
    /* printf("%d:%f/%ld=%f\n", i, tab[i]->sum_neighbour, tab[i]->nb_neighbour, tab[i]->wg); */
  }
}
		 ...
		 double *best_val, tm_tree_t **cur_group, int *nb_groups, int max_groups)
{
  ...
  if( n == arity ){
    ...
    /*evaluate this group*/
    val = eval_grouping(aff_mat, cur_group, arity);
    tm_verbose_fprintf(DEBUG,verbose_stdout,"Grouping %d: %f\n", *nb_groups, val);
...
double fast_grouping(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, tm_tree_t *new_tab_node, int arity, int solution_size, double nb_groups)
{
  ...
  for( l = 0 ; l < solution_size ; l++ ){
    ...
    if(new_tab_node[l].val != best_val){
      tm_verbose_fprintf(CRITICAL,verbose_stderr,"Error: best_val = %f, new_tab_node[%d].val = %f\n", best_val, l, new_tab_node[l].val);
      exit(-1);
    }
  }
...
double k_partition_grouping(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, tm_tree_t *new_tab_node, int arity, int solution_size) {
  ...
  tm_verbose_fprintf(DEBUG,verbose_stdout,"K-Partitionning: n=%d, solution_size=%d, arity=%d\n",n, solution_size,arity);
...
/* Not Called */
double super_fast_grouping(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, tm_tree_t *new_tab_node, int arity, int solution_size)
{
  ...
  global_mat = aff_mat->mat;

  
  
  assert( 2 == arity);

  n = (mat_order*mat_order-mat_order)/2;
  TIC;
  /* graph = (int *)MALLOC(sizeof(int)*2*n); */
  graph = (double *)MALLOC(sizeof(double)*3*n);
  e = 0;
  for( i = 0 ; i < mat_order ; i++ )
    for( j = i+1 ; j < mat_order ; j++){
      ...
      graph[e]= j;
      e++;
      graph[e]= aff_mat->mat[i][j];
      e++;
    }

  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"linearization=%fs (%d==%ld)\n", duration, e, 3*n);


  assert( e == 3*n);
  TIC;
  qsort(graph, n, 3*sizeof(double), tab_cmp);
  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"sorting=%fs\n", duration);
  ...
  l = 0;
  nb_groups = 0;
  for( i = 0 ; (i < 3*n) && (l < solution_size) ; i+=3 )
    if(try_add_edge(tab_node, &new_tab_node[l], arity, (int)graph[i], (int)graph[i+1], &nb_groups))
      l++;
  ...
  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"Grouping=%fs\n", duration);


  tm_verbose_fprintf(INFO,verbose_stdout,"val=%f\n", val);

  display_grouping(new_tab_node, solution_size, arity, val, DEBUG);
...
tm_affinity_mat_t *build_cost_matrix(tm_affinity_mat_t *aff_mat, double* obj_weight, double comm_speed)
{
  ...
  tm_verbose_fprintf(DEBUG,verbose_stdout,"avg=%f\n", avg);
  ...
  return new_affinity_mat(mat, sum_row, mat_order,nnz, 0);
...
void group_nodes(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, tm_tree_t *new_tab_node,
  ...
  */
  ...
  tm_verbose_fprintf(INFO,verbose_stdout,"Number of possible groups:%.0lf\n", nbg);

  /* Todo: check if the depth is a criteria for speeding up the computation*/
  /*  if(nb_groups>30000||depth>5){*/
  if( nbg > tm_get_bucket_grouping_threshold()*1000.0){
    double duration;

    TIC;
    if( arity <= 2 ){
      /* tm_tree_t* new_tab_node2 = (tm_tree_t*)MALLOC(sizeof(tm_tree_t)*solution_size); */
      /* memcpy(new_tab_node2,new_tab_node,sizeof(tm_tree_t)*solution_size); */
      /* tm_verbose_fprintf(INFO,verbose_stdout,"Super Fast  Grouping...\n"); */
      /* TIC; */
      /* val = super_fast_grouping(cost_mat, tab_node, new_tab_node2, arity, solution_size); */
      /* duration = TOC; */
      /* tm_verbose_fprintf(INFO,verbose_stdout,"Val = %f in %f\n",val,duration); */
      tm_verbose_fprintf(INFO,verbose_stdout,"Bucket Grouping...\n");
      TIC;
      val = bucket_grouping(cost_mat, tab_node, new_tab_node, arity, solution_size);
      duration = TOC;
      tm_verbose_fprintf(INFO,verbose_stdout,"TM Grouping done (Val = %f) in %f\n",val,duration);
    }else if( arity <= 5){
      tm_verbose_fprintf(INFO,verbose_stdout,"Fast Grouping...\n");
      val = fast_grouping(cost_mat, tab_node, new_tab_node, arity, solution_size, nbg);
    } else{
      tm_verbose_fprintf(INFO,verbose_stdout,"K-partition Grouping...\n");
      val = k_partition_grouping(cost_mat, tab_node, new_tab_node, arity, solution_size);
    }

    duration = TOC;
    tm_verbose_fprintf(INFO,verbose_stdout,"Fast grouping duration=%f\n", duration);

    display_grouping(new_tab_node, solution_size, arity, val, INFO);

  }else{
    unsigned long int  nb_groups = (unsigned long int) nbg;
    tm_verbose_fprintf(INFO,verbose_stdout,"Grouping nodes...\n");
    list.next = NULL;
    ...
    tm_verbose_fprintf(INFO,verbose_stdout,"List to tab done\n");
    ...
    if(verbose_level>=INFO)
      if(timeout)
	tm_verbose_fprintf(INFO,verbose_stdout,"Packed mapping timeout!\n");
    ...
    best_val /= 1.001;
    /* best_val *= 1.001; */
    tm_verbose_fprintf(INFO,verbose_stdout,"Packing computed\n");
    ...
    if(verbose_level>=INFO){
      if(timeout){
	tm_verbose_fprintf(INFO,verbose_stdout,"Cost less first timeout!\n");
      }
      if(last_best>best_val){
	tm_verbose_fprintf(INFO,verbose_stdout,"Cost less first Impoved solution\n");
      }
    }
    ...
    if(verbose_level>=INFO){
      if(timeout)
	tm_verbose_fprintf(INFO,verbose_stdout,"Cost most last timeout!\n");
      if(last_best>best_val)
	tm_verbose_fprintf(INFO,verbose_stdout,"Cost most last impoved solution\n");
    }
    if( nb_groups < 1000000 ){
      /* perform a mapping in the weighted degree order */

      tm_verbose_fprintf(INFO,verbose_stdout,"----WG----\n");


      compute_weighted_degree(tab_group, nb_groups, arity);

      tm_verbose_fprintf(INFO,verbose_stdout,"Weigted degree computed\n");
	...
	tab_group[i]->id = i;
      ...
      if(verbose_level>=INFO){
	if(timeout)
	  tm_verbose_fprintf(INFO,verbose_stdout,"WG timeout!\n");
	if(last_best>best_val)
	  tm_verbose_fprintf(INFO,verbose_stdout,"WG impoved solution\n");
      }
      tm_verbose_fprintf(INFO,verbose_stdout,"----WG done----\n");
      
    }

    if(tm_get_exhaustive_search_flag()){
      tm_verbose_fprintf(INFO,verbose_stdout,"Running exhaustive search on %ld groups, please wait...\n",nb_groups);
      ...
      if(verbose_level>=INFO){
	if(last_best>best_val){
	  tm_verbose_fprintf(INFO,verbose_stdout,"Exhaustive search improved solution by: %.3f\n",(last_best-best_val)/last_best);
	} else {
	  tm_verbose_fprintf(INFO,verbose_stdout,"Exhaustive search did not improved solution\n");
	}
      }
    }
  ...
  tm_verbose_fprintf(INFO,verbose_stdout, "Grouping done in %.4fs!\n", duration);
}
...
void complete_aff_mat(tm_affinity_mat_t **aff_mat , int mat_order, int K)
{
  ...
  *aff_mat = new_affinity_mat(new_mat, sum_row, M, (*aff_mat)->nnz, (*aff_mat)->max_val);
}
...
void create_dumb_tree(tm_tree_t *node, int depth, tm_topology_tree_t *topology)
{
...
void complete_tab_node(tm_tree_t **tab, int mat_order, int K, int depth, tm_topology_tree_t *topology)
{
...
*/
tm_tree_t *build_level_topology(tm_tree_t *tab_node, tm_affinity_mat_t *aff_mat, int arity, int depth, tm_topology_tree_t *topology,
				double *obj_weight, double *comm_speed)
{
  ...
  if( 0 == depth ){
    if((1 == mat_order) && (0 == depth))
      ...
      tm_verbose_fprintf(CRITICAL,verbose_stderr,"Error: matrix size: %d and depth:%d (should be 1 and -1 respectively)\n", mat_order, depth);
      exit(-1);
    }
  }
  ...
  if( mat_order%arity != 0 ){
    ...
    tm_verbose_fprintf(INFO,verbose_stdout,"****mat_order=%d arity=%d K=%d\n", mat_order, arity, K);
    /*display_tab(tab, mat_order);*/
    ...
    duration = TOC;
    tm_verbose_fprintf(INFO,verbose_stdout,"Completing matrix duration= %fs\n", duration);
  } /*display_tab(tab, mat_order);*/

  M = mat_order/arity;
  tm_verbose_fprintf(INFO,verbose_stdout,"Depth=%d\tnb_nodes=%d\tnb_groups=%d\tsize of groups(arity)=%d\n", depth, mat_order, M, arity);
  ...
  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"New nodes creation= %fs\n", duration);
  ...
  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout,"Aggregate_com_mat= %fs\n", duration);
  TIC;
  ...
  duration = TOC;
  tm_verbose_fprintf(INFO,verbose_stdout, "Aggregate obj_weight= %fs\n", duration);
...
tm_tree_t *bottom_up_build_tree_from_topology(tm_topology_tree_t *topology, tm_affinity_mat_t *aff_mat,
					      double *obj_weight, double *comm_speed){
  int depth, i;
  ...
  tm_verbose_fprintf(INFO,verbose_stdout,"nb_levels=%d\n", depth);
  /* assume all objects have the same arity*/
  res = build_level_topology(tab_node, aff_mat , topology->arity[depth-2], depth-1, topology, obj_weight, comm_speed);
  tm_verbose_fprintf(INFO,verbose_stdout,"Build (top down) tree done!\n");
...
int check_constraints(tm_topology_tree_t  *topology, int **constraints)
{

  int sorted = 1;
  int last = -1;
  int i, shift;
  int nb_constraints = topology->common->nb_constraints*topology->common->oversub_fact;
  if(nb_constraints && topology->common->constraints){
    *constraints = (int*)MALLOC(sizeof(int)*(nb_constraints));
    ...
    for(i = 0 ; i < nb_constraints ; i++){
      ...
      */
      shift = 1 + i%topology->common->oversub_fact - topology->common->oversub_fact;
      (*constraints)[i] = topology->node_rank[topology->common->constraints[i/topology->common->oversub_fact]] +shift;
...
tm_tree_t * tm_build_tree_from_topology(tm_topology_tree_t *topology, tm_affinity_mat_t *aff_mat, double *obj_weight, double *com_speed)
{
  ...
  oversub_fact   = topology->common->oversub_fact;
  ...
  npu            = nb_processing_units_tree(topology);
  nb_slots       = npu * oversub_fact;

  if(verbose_level >= INFO){
    tm_verbose_fprintf(INFO,  verbose_stdout, "Com matrix size      : %d\n", nb_processes);
    tm_verbose_fprintf(INFO,  verbose_stdout, "nb_constraints       : %d", nb_constraints);
    if(constraints){
      tm_verbose_fprintf(INFO,  verbose_stdout," -> ");
    ...
    tm_verbose_fprintf(INFO,  verbose_stdout, "nb_processing units  : %d\n", npu);
    tm_verbose_fprintf(INFO,  verbose_stdout, "Oversubscrbing factor: %d\n", oversub_fact);
    tm_verbose_fprintf(INFO,  verbose_stdout, "Nb of slots          : %d\n", nb_slots);
  }

  if(nb_processes > nb_constraints){
    tm_verbose_fprintf(CRITICAL,verbose_stderr, "Error : Not enough slots/constraints (%d) for the communication matrix order (%d)!\n", nb_constraints, nb_processes);
    exit(-1);
  }

  if(nb_constraints == nb_slots)
    {
      tm_verbose_fprintf(INFO,verbose_stdout,"No need to use %d constraints for %d slots!\n", nb_constraints, nb_slots);
      nb_constraints = 0;
      FREE(constraints);
    }

  if(nb_constraints){
    tm_verbose_fprintf(INFO,verbose_stdout,"Partitionning with constraints\n");

    result = kpartition_build_tree_from_topology(topology, aff_mat->mat,  nb_processes, aff_mat->max_val, constraints, nb_constraints,
    ...
    tm_verbose_fprintf(INFO,verbose_stdout,"Partitionning without constraints\n");
